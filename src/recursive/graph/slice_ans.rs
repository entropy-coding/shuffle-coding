...
use crate::ans::{Bernoulli, Codec, ConstantCodec, LogUniform, Message, MutCategorical, MutDistribution, Symbol};
use crate::graph::{Directed, EdgeIndex, EdgeType, Graph};
use crate::graph_ans::{EdgeIndicesCodec, EdgesIID, ErdosRenyi};
use crate::multiset::Multiset;
use crate::permutable::{Permutable, Unordered};
use crate::recursive::{FromPrefix, OrdSymbol, Prefix, SliceCodecs};
use crate::recursive::complete::{Len, multiset_shuffle_codec, MultisetShuffleCodec};
...
impl<NodeC: Codec + Symbol, EdgeC: Codec<Symbol: OrdSymbol> + Symbol, Ty: EdgeType> FromPrefix for ErdosRenyiSliceCodecs<NodeC, EdgeC, Ty> {
...
impl<N: Symbol, E: Symbol, Ty: EdgeType, GraphSliceCs: FromPrefix<Prefix=GraphPrefix<N, E, Ty>,
    Output: Codec<Symbol=<Self::Prefix as Prefix>::Slice>> + Len> SliceCodecs for GraphSliceCs {
    type SliceCodec = Self::Output;
    type EmptyPrefixCodec = ConstantCodec<Self::Prefix>;

    fn empty_prefix(&self) -> Self::EmptyPrefixCodec {
        let graph = Graph::empty(repeat(None).take(self.len()));
...
impl<NodeC: Codec + Symbol, EdgeC: Codec<Symbol: OrdSymbol> + Symbol, Ty: EdgeType> FromPrefix for PolyaUrnSliceCodecs<NodeC, EdgeC, Ty> {
    type Prefix = GraphPrefix<NodeC::Symbol, EdgeC::Symbol, Ty>;
