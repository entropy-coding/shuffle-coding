...
use crate::graph::{Directed, Edge, EdgeType, Graph};
use crate::multiset::Multiset;
use crate::permutable::{Permutable, Unordered};
use crate::plain::{Automorphisms, GraphPlainPermutable, PlainPermutable};
...
impl<N: Symbol, E: Symbol, Ty: EdgeType> Prefix for GraphPrefix<N, E, Ty> {
    ...
    fn pop_slice(&mut self) -> Self::Slice {
        ...
        let x = &mut self.graph.nodes[new_unknown_node].0;
        let label = x.clone().unwrap();
        *x = None;
        // TODO remove once MutCategorical tree is balanced on update:
        edges.shuffle(&mut thread_rng());
        (label, Unordered(edges))
    }
