...
use crate::graph_ans::{EdgeIndicesCodec, GraphIID, PolyaUrnEdgeIndexCodec, PolyaUrnEdgeIndicesCodec};
use crate::permutable::{Orbit, Permutable, Unordered};
use crate::recursive::{FromPrefix, hash, JointPrefix, OrbitCodec, orbits_by_id, Prefix, PrefixOrbitCodec};
use crate::recursive::complete::Len;
use crate::recursive::graph::prefix::GraphPrefix;
use crate::recursive::graph::slice_ans::PolyaUrnSliceEdgeIndicesCodec;
use crate::recursive::OrdSymbol;
...
impl<P: Prefix<Full=Graph<N, E, Ty>>, N: OrdSymbol, E: OrdSymbol, Ty: EdgeType> WLOrbitCodecs<P, N, E, Ty> {
    ...
    fn update_around_slice(&self, x: &GraphPrefix<N, E, Ty>, orbits: &mut PrefixOrbitCodec<Vec<usize>>, (_, Unordered(edges)): &<GraphPrefix<N, E, Ty> as Prefix>::Slice, index: usize) {
        ...
        self.update_around(&x.graph, orbits, neighbors, false);
    }

    fn update_around<M: OrdSymbol>(&self, x: &Graph<M, E, Ty>, orbits: &mut PrefixOrbitCodec<Vec<usize>>, elements: Vec<usize>, node_hash_some: bool) {
        assert!(!Ty::is_directed());
        // Add elements at distance 0.
        let mut new_ids_by_distance = vec![BTreeMap::from_iter(elements.into_iter().map(|id| (id, orbits.id(id))))];
        ...
        for (&i, new_id) in new_ids_by_distance[0].iter_mut() {
            ...
            let mut h = if node_hash_some { hash((Some(n), index)) } else { hash((n, index)) };
...
impl<N: OrdSymbol, E: OrdSymbol, Ty: EdgeType> FromPrefix for WLOrbitCodecs<JointPrefix<Graph<N, E, Ty>>, N, E, Ty> {
    ...
    fn update_after_pop_slice(&self, orbits: &mut Self::Output, x: &Self::Prefix, _slice: &()) {
        ...
        orbits.pop_id();
        self.update_around(&x.full, orbits, vec![x.len()], true);
    }
    ...
    fn update_after_push_slice(&self, orbits: &mut Self::Output, x: &Self::Prefix, _slice: &()) {
        ...
        orbits.push_id();
        self.update_around(&x.full, orbits, vec![x.last_()], true);
    }
...
impl<N: OrdSymbol, E: OrdSymbol, Ty: EdgeType> FromPrefix for WLOrbitCodecs<GraphPrefix<N, E, Ty>, N, E, Ty> {
    ...
    fn update_after_push_slice(&self, oribts: &mut Self::Output, x: &Self::Prefix, slice: &<Self::Prefix as Prefix>::Slice) {
        if Ty::is_directed() {
            *oribts = self.apply(x);
            return;
        }
        ...
        oribts.push_id();
        self.update_around_slice(x, oribts, &slice, x.last_());
    }
...
pub mod tests {
    ...
    use crate::graph_ans::tests::{plain_erdos_renyi, test_graph_shuffle_codecs};
    ...
    use crate::recursive::PrefixOrbitCodec;
