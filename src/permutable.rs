...
use crate::shuffle_ans::{AutomorphismUniform, PermutationUniform, RCosetUniform, Unordered};

#[derive(Clone, Debug, Eq, Hash, Ord, PartialEq, PartialOrd)]
pub struct Permutation(pub BTreeMap<usize, usize>);

impl Permutation {
    pub fn identity() -> Permutation { Permutation(BTreeMap::new()) }

    pub fn inverse(&self) -> Permutation {
        Permutation(self.0.iter().map(|(&x, &y)| (y, x)).collect())
    }

    pub fn from(items: impl IntoIterator<Item=usize>) -> Permutation {
        Permutation(items.into_iter().enumerate().filter(|(i, x)| i != x).map(|(i, x)| (i, x)).collect())
    }
    ...
    pub fn is_identity(&self) -> bool { self.0.is_empty() }

    fn normalize(&mut self) {
        self.0.retain(|i, x| i != x);
    }

    pub fn assert_len(&self, len: usize) {
        assert!(self.0.iter().all(|(&x, &y)| x < len && y < len));
    }

    pub fn swap(x: usize, y: usize) -> Permutation {
        Permutation(if x == y { BTreeMap::new() } else { BTreeMap::from_iter(vec![(x, y), (y, x)]) })
    }

    pub fn swap_elements(&mut self, x: usize, y: usize) {
        let p = self as &Permutation;
        if x == y { return; }
        let px = p * x;
        let py = p * y;
        if y == px { self.0.remove(&y); } else { self.0.insert(y, px); }
        if x == py { self.0.remove(&x); } else { self.0.insert(x, py); }
    }
}
...
impl Mul<Permutation> for Permutation {
    ...
    fn mul(mut self, mut rhs: Permutation) -> Permutation {
        for x in rhs.0.values_mut() {
            *x = self.0.remove(x).unwrap_or(*x);
        }

        for (&i, &x) in self.0.iter() {
            rhs.0.entry(i).or_insert(x);
        }
...
impl Automorphism {
    ...
    pub fn total(self) -> Permutation {
        let mut total = Permutation::identity();
...
impl PermutationGroup {
    ...
    pub fn orbit_stabilizer(&self, element: usize) -> OrbitStabilizerInfo {
        ...
        let mut from = HashMap::from([(element, Permutation::identity())]);
...
pub trait Permutable: Symbol {
    ...
    fn test_left_group_action_axioms(&self, seed: usize) {
        ...
        assert_eq!(&(&Permutation::identity() * self), self);
...
pub trait Partial: Permutable {
    ...
    fn push(&mut self, diff: Self::Diff);
    fn empty(len: usize) -> Self;
    fn from_complete(complete: Self::Complete) -> Self;
    fn into_complete(self) -> Self::Complete;
    fn last(&self) -> usize { self.len() - 1 }
}
...
impl<P: Partial, Q: Partial> Partial for (P, Q) {
    ...
    type Diff = (P::Diff, Q::Diff);

    fn pop(&mut self) -> Self::Diff {
        (self.0.pop(), self.1.pop())
    }
...
pub mod tests {
    ...
    fn permutation_elements() {
        let p0 = Permutation(BTreeMap::from([(2, 3), (3, 4), (4, 2)]));
        let p1 = Permutation(BTreeMap::from([(0, 1), (1, 3), (2, 0), (3, 2)]));
        let p2 = p1 * p0;
        assert_eq!(p2.0, BTreeMap::from([(0, 1), (1, 3), (3, 4), (4, 0)]));
    }
    ...
    fn permutation_axioms() {
        for i in 0..20 {
            ...
            assert_eq!(Permutation::identity() * p0.clone(), p0);
            assert_eq!(p0.inverse() * p0.clone(), Permutation::identity());
            assert_eq!(p0.clone() * p0.inverse(), Permutation::identity());
            assert_eq!((p2.clone() * p1.clone()) * p0.clone(), p2.clone() * (p1.clone() * p0.clone()));

            // Compatibility with integer:
            assert_eq!(&Permutation::identity() * 2, 2);
