...
use crate::ans::{Bernoulli, Categorical, Codec, CodecTestResults, Independent, Message, Symbol, Uniform, UniformCodec};
use crate::datasets::{Dataset, SourceInfo};
use crate::graph::{EdgeType, Graph, Undirected};
use crate::graph_ans::{EmptyCodec, erdos_renyi_indices, GraphIID, num_all_edge_indices, polya_urn};
use crate::param_ans::{AutoregressiveErdosReyniGraphDatasetParamCodec, AutoregressivePolyaUrnGraphDatasetParamCodec, CategoricalParamCodec, EmptyParamCodec, ErdosRenyiParamCodec, GraphDatasetParamCodec, ParametrizedIndependent, PolyaUrnParamCodec, UniformParamCodec, WrappedParametrizedIndependent};
use crate::permutable::{Permutable, Unordered};
use crate::permutable::PermutationUniform;
use crate::plain::{plain_shuffle_codec, PlainPermutable, with_isomorphism_test_max_len};
use crate::plain::coset_recursive::coset_recursive_shuffle_codec;
use crate::plain::labelled_graph::EdgeLabelledGraph;
use crate::recursive::{Autoregressive, JointPrefix, JointSliceCodecs, OrdSymbol, ShuffleCodec, SliceCodecs};
use crate::recursive::complete::{CompleteOrbitCodecs, Len};
use crate::recursive::graph::incomplete::WLOrbitCodecs;
use crate::recursive::graph::prefix::GraphPrefix;
use crate::recursive::graph::slice_ans::{ErdosRenyiSliceCodecs, PolyaUrnSliceCodecs};
...
impl DatasetBenchmark<'_> {
    ...
    fn run_autoregressive_model<N: OrdSymbol, E: OrdSymbol, Ty: EdgeType, S: SliceCodecs<Prefix=GraphPrefix<N, E, Ty>> + Symbol + Len>(
        &self,
        ...
        with_isomorphism_test_max_len(self.config.isomorphism_test_max_len, || {
            ...
            if self.config.complete {
                let codec = WrappedParametrizedIndependent {
                    ...
                    codec_from_inner: Box::new(move |x| Independent::new(x.codecs.iter().cloned().map(|c| ShuffleCodec::new(c.0, CompleteOrbitCodecs::new())))),
                };
                if self.config.unparametrized {
    ...
    fn run_joint_model<N: OrdSymbol, E: OrdSymbol, Ty: EdgeType, C: Codec<Symbol=Graph<N, E, Ty>> + Symbol + Len>(
        &self,
        codec: ParametrizedIndependent<impl Codec<Symbol=Independent<C>>, impl Fn(&Vec<C::Symbol>) -> Independent<C> + Clone>,
        graphs: &Vec<C::Symbol>) where
        Graph<N, E, Ty>: PlainPermutable,
        GraphPrefix<N, E, Ty>: PlainPermutable,
    {
        with_isomorphism_test_max_len(self.config.isomorphism_test_max_len, || {
            ...
            if self.config.joint {
                if self.config.complete {
                    let codec = WrappedParametrizedIndependent {
                        parametrized_codec: codec.clone(),
                        data_to_inner: to_ordered.clone(),
                        codec_from_inner: Box::new(move |x| Independent::new(x.codecs.iter().cloned().map(|c| ShuffleCodec::new(JointSliceCodecs::new(c), CompleteOrbitCodecs::new())))),
                    };
                    if self.config.unparametrized {
                        test_and_print(&(codec.codec_from_inner)(param.clone()), &unordered, &self.config.initial_message());
                    }
