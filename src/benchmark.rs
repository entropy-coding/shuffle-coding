...
use crate::graph_ans::{CategoricalParamCodec, EmptyCodec, EmptyParamCodec, erdos_renyi_indices, ErdosRenyiParamCodec, GraphDatasetParamCodec, GraphIID, num_all_edge_indices, ParametrizedIndependent, polya_urn, PolyaUrnParamCodec, UniformParamCodec, WrappedParametrizedIndependent};
use crate::permutable::{GroupPermutable, Permutable};
use crate::shuffle_ans::{PermutationUniform, shuffle_codec, TestConfig, Unordered};
...
pub struct Config {
    ...
    /// SZIP:https://raw.githubusercontent.com/entropy-coding/shuffle-coding/main/szip-graphs/datasets.md.
...
impl DatasetBenchmark<'_> {
    fn er_indices(&self) -> impl Fn(Vec<usize>, bool) -> ErdosRenyiParamCodec + '_ + Clone {
        |nums_nodes, loops| ErdosRenyiParamCodec::new(nums_nodes, self.config.undirected, loops)
    }
    fn pu_indices(&self, original: bool) -> impl Fn(Vec<usize>, bool) -> PolyaUrnParamCodec + '_ + Clone {
        move |nums_nodes, loops| PolyaUrnParamCodec::new(nums_nodes, self.config.undirected, loops || original, original)
    }
    ...
    fn verify_and_print_stats(&self, stats: &DatasetStats) {
        ...
        let (node_entropy, node_labels) = if let Some(n) = &stats.node_label { (n.entropy(), n.codecs.len()) } else { (0., 0) };
        let (edge_entropy, edge_labels) = if let Some(n) = &stats.edge_label { (n.entropy(), n.codecs.len()) } else { (0., 0) };
    ...
    pub fn run(&self) {
        ...
        if self.config.plain || !(self.dataset.has_node_labels || self.dataset.has_edge_labels) {
            ...
            self.verify_and_print_stats(&DatasetStats::unlabelled(&graphs, self.config.undirected));
            let er = || self.run_model(ParametrizedIndependent {
                param_codec: GraphDatasetParamCodec { node: EmptyParamCodec::default(), edge: EmptyParamCodec::default(), indices: self.er_indices(), undirected: self.config.undirected },
                infer: |graphs| {
                    let DatasetStats { loops, edge, .. } = DatasetStats::unlabelled(&graphs, self.config.undirected);
                    VecCodec::new(graphs.iter().map(|x| {
                        GraphIID::new(x.len(), erdos_renyi_indices(x.len(), edge.clone(), self.config.undirected, loops), EmptyCodec::default(), EmptyCodec::default(), self.config.undirected)
                    }).collect_vec())
                },
            }, &graphs);
            ...
            for original in self.config.pu_models_original() {
                self.run_model(ParametrizedIndependent {
                    param_codec: GraphDatasetParamCodec { node: EmptyParamCodec::default(), edge: EmptyParamCodec::default(), indices: self.pu_indices(original), undirected: self.config.undirected },
                    infer: |graphs| {
                        let DatasetStats { loops, .. } = DatasetStats::unlabelled(&graphs, self.config.undirected);
                        VecCodec::new(graphs.iter().map(|x| {
                            let edge_indices = polya_urn(x.len(), x.num_edges(self.config.undirected), self.config.undirected, loops || original, original);
                            GraphIID::new(x.len(), edge_indices, EmptyCodec::default(), EmptyCodec::default(), self.config.undirected)
                        }).collect_vec())
                    },
                }, &graphs);
            }
            ...
            self.verify_and_print_stats(&DatasetStats::edge_labelled(&graphs, self.config.undirected));

            if self.config.er {
                self.run_model(ParametrizedIndependent {
                    param_codec: GraphDatasetParamCodec { node: CategoricalParamCodec, edge: CategoricalParamCodec, indices: self.er_indices(), undirected: self.config.undirected },
                    infer: |graphs| {
                        let DatasetStats { loops, edge, node_label, edge_label, .. } = DatasetStats::edge_labelled(&graphs, self.config.undirected);
                        VecCodec::new(graphs.iter().map(|x| GraphIID::new(
                            x.len(), erdos_renyi_indices(x.len(), edge.clone(), self.config.undirected, loops), node_label.clone().unwrap(), edge_label.clone().unwrap(), self.config.undirected)).collect_vec())
                    },
                }, &graphs);
            }
            if self.config.uniform_er {
                self.run_model(ParametrizedIndependent {
                    param_codec: GraphDatasetParamCodec { node: UniformParamCodec::default(), edge: UniformParamCodec::default(), indices: self.er_indices(), undirected: self.config.undirected },
                    infer: |graphs| {
                        let DatasetStats { loops, edge, node_label, edge_label, .. } = DatasetStats::edge_labelled(&graphs, self.config.undirected);
                        let node_label = Uniform::new(node_label.unwrap().codecs.len());
                        let edge_label = Uniform::new(edge_label.unwrap().codecs.len());
                        VecCodec::new(graphs.iter().map(|x| GraphIID::new(
                            x.len(), erdos_renyi_indices(x.len(), edge.clone(), self.config.undirected, loops), node_label.clone(), edge_label.clone(), self.config.undirected)).collect_vec())
                    },
                }, &graphs);
            }
            for original in self.config.pu_models_original() {
                self.run_model(ParametrizedIndependent {
                    param_codec: GraphDatasetParamCodec { node: CategoricalParamCodec, edge: CategoricalParamCodec, indices: self.pu_indices(original), undirected: self.config.undirected },
                    infer: |graphs| {
                        let DatasetStats { loops, node_label, edge_label, .. } = DatasetStats::edge_labelled(&graphs, self.config.undirected);
                        VecCodec::new(graphs.iter().map(|x| {
                            let edge_indices = polya_urn(x.len(), x.num_edges(self.config.undirected), self.config.undirected, loops || original, original);
                            GraphIID::new(x.len(), edge_indices, node_label.clone().unwrap(), edge_label.clone().unwrap(), self.config.undirected)
                        }).collect_vec())
                    },
                }, &graphs);
            }
            ...
            print_flush!("nodes ");
            let graphs = self.sorted(graphs.clone());
            self.verify_and_print_stats(&DatasetStats::node_labelled(&graphs, self.config.undirected));
            if self.config.er {
                self.run_model(ParametrizedIndependent {
                    param_codec: GraphDatasetParamCodec { node: CategoricalParamCodec, edge: EmptyParamCodec::default(), indices: self.er_indices(), undirected: self.config.undirected },
                    infer: |graphs| {
                        let DatasetStats { loops, edge, node_label, .. } = DatasetStats::node_labelled(&graphs, self.config.undirected);
                        VecCodec::new(graphs.iter().map(|x| {
                            let edge_indices = erdos_renyi_indices(x.len(), edge.clone(), self.config.undirected, loops);
                            GraphIID::new(x.len(), edge_indices, node_label.clone().unwrap(), EmptyCodec::default(), self.config.undirected)
                        }).collect_vec())
                    },
                }, &graphs);
            }
            if self.config.uniform_er {
                self.run_model(ParametrizedIndependent {
                    param_codec: GraphDatasetParamCodec { node: UniformParamCodec::default(), edge: EmptyParamCodec::default(), indices: self.er_indices(), undirected: self.config.undirected },
                    infer: |graphs| {
                        let DatasetStats { loops, edge, node_label, .. } = DatasetStats::node_labelled(&graphs, self.config.undirected);
                        let node_label = Uniform::new(node_label.unwrap().codecs.len());
                        VecCodec::new(graphs.iter().map(|x| {
                            let edge_indices = erdos_renyi_indices(x.len(), edge.clone(), self.config.undirected, loops);
                            GraphIID::new(x.len(), edge_indices, node_label.clone(), EmptyCodec::default(), self.config.undirected)
                        }).collect_vec())
                    },
                }, &graphs);
            }
            for original in self.config.pu_models_original() {
                self.run_model(ParametrizedIndependent {
                    param_codec: GraphDatasetParamCodec { node: CategoricalParamCodec, edge: EmptyParamCodec::default(), indices: self.pu_indices(original), undirected: self.config.undirected },
                    infer: |graphs| {
                        let DatasetStats { loops, node_label, .. } = DatasetStats::node_labelled(&graphs, self.config.undirected);
                        VecCodec::new(graphs.iter().map(|x| {
                            let edge_indices = polya_urn(x.len(), x.num_edges(self.config.undirected), self.config.undirected, loops || original, original);
                            GraphIID::new(x.len(), edge_indices, node_label.clone().unwrap(), EmptyCodec::default(), self.config.undirected)
                        }).collect_vec())
    ...
    fn sorted<N: Symbol, E: Symbol>(&self, mut graphs: Vec<Graph<N, E>>) -> Vec<Graph<N, E>> where Graph<N, E>: GroupPermutable {
...
impl DatasetStats {
    pub fn unlabelled<N: Symbol, E: Symbol>(graphs: &Vec<Graph<N, E>>, undirected: bool) -> Self {
        let num_graphs = graphs.len();
        let loops = graphs.iter().any(|x| x.has_selfloops());
        let total_nodes = graphs.iter().map(|x| x.len()).sum::<usize>();
        let total_edges = graphs.iter().map(|x| x.num_edges(undirected)).sum::<usize>();
        let total_possible_edges = graphs.iter().map(|x| num_all_edge_indices(x.len(), undirected, loops)).sum::<usize>();
    ...
    pub fn node_labelled<E: Symbol>(graphs: &Vec<Graph<usize, E>>, undirected: bool) -> Self {
        Self { node_label: Some(Self::node_label_dist(graphs)), ..Self::unlabelled(graphs, undirected) }
    }

    pub fn edge_labelled(graphs: &Vec<Graph<usize, usize>>, undirected: bool) -> Self {
        Self { edge_label: Some(Self::edge_label_dist(graphs)), ..Self::node_labelled(graphs, undirected) }
    }

    fn edge_label_dist(graphs: &Vec<Graph<usize, usize>>) -> Categorical {
        Self::dist(&graphs.iter().flat_map(|x| x.edge_labels()).counts())
    }

    fn node_label_dist<E: Symbol>(graphs: &Vec<Graph<usize, E>>) -> Categorical {
        Self::dist(&graphs.iter().flat_map(|x| x.node_labels()).counts())
    }

    fn dist(counts: &HashMap<usize, usize>) -> Categorical {
        Categorical::new(&(0..counts.keys().max().unwrap() + 1).map(|x| counts.get(&x).cloned().unwrap_or_default()).collect_vec())
    }
