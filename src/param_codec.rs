...
use crate::codec::{Bernoulli, Categorical, Codec, ConstantCodec, IID, Independent, LogUniform, Message, OrdSymbol, Symbol, Uniform};
use crate::graph::{Directed, EdgeType};
use crate::graph_codec::{AllEdgeIndices, EdgeIndicesCodec, EmptyCodec, erdos_renyi_indices, ErdosRenyi, GraphIID, num_all_edge_indices, polya_urn, PolyaUrn};
use crate::recursive::Autoregressive;
...
impl MaxBenfordIID {
    ...
    fn bits_codec() -> &'static LogUniform {
        lazy_static! {static ref C: LogUniform = LogUniform::new(6);}
        &C
    }
...
impl<
    Ty: EdgeType,
    NodeParamC: Codec<Symbol: Codec<Symbol: Default>>,
    EdgeParamC: Codec<Symbol: Codec<Symbol: OrdSymbol>>,
...
Codec for AutoregressiveErdosReyniGraphDatasetParamCodec<IndicesParamCFromNumsNodesAndLoops, NodeParamC, EdgeParamC, ErdosRenyi<Ty>, IndicesParamC> {
    type Symbol = Independent<Autoregressive<ErdosRenyiSliceCodecs<NodeParamC::Symbol, EdgeParamC::Symbol, Ty>>>;

    fn push(&self, m: &mut Message, x: &Self::Symbol) {
        let er = Independent::new(x.codecs.iter().map(|c| &c.0).map(
    ...
    fn pop(&self, m: &mut Message) -> Self::Symbol {
        Independent::new(self.joint.pop(m).codecs.into_iter().map(
            |c| Autoregressive(ErdosRenyiSliceCodecs {
                len: c.nodes.len,
...
Codec for AutoregressivePolyaUrnGraphDatasetParamCodec<NodeParamC, EdgeParamC, Ty> {
    type Symbol = Independent<Autoregressive<PolyaUrnSliceCodecs<NodeParamC::Symbol, EdgeParamC::Symbol, Ty>>>;

    fn push(&self, m: &mut Message, x: &Self::Symbol) {
        let first = &x.codecs.first().unwrap().0;
        self.edge.push(m, &first.edge);
        self.node.push(m, &first.node);
        Bernoulli::new(1, 2).push(m, &first.loops);
        let nums_nodes = x.codecs.iter().map(|x| x.0.len).rev().collect_vec();
        SortedDiffRunLengthCodec::default().push(m, &nums_nodes);
    }
    ...
    fn pop(&self, m: &mut Message) -> Self::Symbol {
        ...
        Autoregressive(PolyaUrnSliceCodecs { len: n, node: node.clone(), edge: edge.clone(), loops, phantom: PhantomData })))
    }
...
impl<ParamC: Codec<Symbol: Codec>,
    Infer: Fn(&<ParamC::Symbol as Codec>::Symbol) -> ParamC::Symbol + Clone,
    C: Symbol + Codec> Clone for WrappedParametrizedIndependent<ParamC, Infer, C> {
    fn clone(&self) -> Self {
        unimplemented!()
    }
}
...
#[cfg(test)]
pub mod tests {
    use crate::codec::{Bernoulli, Categorical, Codec, ConstantCodec, Independent, Message, Uniform};
    use crate::graph::Undirected;
    use crate::graph_codec::{EmptyCodec, erdos_renyi_indices, GraphIID};
