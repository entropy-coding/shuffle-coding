    ...
    pub fn defaults() -> Vec<SourceInfo> {
        lazy_static! {static ref D: Vec<SourceInfo> = vec![
            SourceInfo::new(
                ...
                Some("https://chrsmrrs.github.io/datasets/docs/datasets/")),
            SourceInfo::new(
                "SZIP",
                "https://raw.githubusercontent.com/entropy-coding/shuffle-coding/main/szip-graphs/datasets.md",
                Some("https://raw.githubusercontent.com/entropy-coding/shuffle-coding")),
        ];}
        D.clone()
    }
}
...
impl Dataset {
    ...
    pub fn unlabelled_graphs(&self) -> Vec<Graph> {
        self.graphs_with_edge_order().into_iter().map(|(g, _)| g).collect_vec()
    }

    pub fn node_labelled_graphs(&self) -> Option<Vec<NodeLabelledGraph>> {
        self.node_labelled_graphs_with_edge_order().map(|x| x.into_iter().map(|(g, _)| g).collect_vec())
    }

    pub fn edge_labelled_graphs(&self) -> Option<Vec<EdgeLabelledGraph>> {
        ...
        for (x, edge_indices) in graphs_with_edge_order {
            ...
            let graph = EdgeLabelledGraph::new(x.node_labels(), edges);
    ...
    fn graphs_with_edge_order(&self) -> Vec<(Graph, Vec<EdgeIndex>)> {
        ...
        let graphs = nodes_by_graph.into_iter().map(|(g, nodes)| {
            ...
            (Graph::plain(nodes.len(), es.clone()), es)
        }).collect_vec();
    ...
    fn verify_graphs<'a>(&self, graphs: &Vec<&'a Graph>) {
        ...
        let avg_edges = graphs.iter().map(|g| g.num_edges(true)).sum::<usize>() as f64 / num_graphs as f64;
    ...
    fn node_labelled_graphs_with_edge_order(&self) -> Option<Vec<(NodeLabelledGraph, Vec<EdgeIndex>)>> {
        ...
        for (g, edge_indices) in graphs_with_order {
            ...
            start += g.len();
            let vtg = NodeLabelledGraph::new(node_labels, g.edges());
            out.push((vtg, edge_indices));
        }
...
mod tests {
    ...
    use crate::shuffle_ans::{TestConfig, Unordered};
    use crate::shuffle_ans::test::test_shuffle_codecs;

    #[test]
    fn unlabelled() {
        let graphs = dataset("MUTAG").unlabelled_graphs();
        let DatasetStats { edge, .. } = DatasetStats::unlabelled(&graphs, true);
        let codec = VecCodec::new(graphs.iter().map(|x| {
            GraphIID::new(x.len(), erdos_renyi_indices(x.len(), edge.clone(), true, false), EmptyCodec::default(), EmptyCodec::default(), true)
        }).collect_vec());
        ...
        for loops in [false, true] {
            for redraws in [false, true] {
                let pu = &graphs.iter().map(|x| {
                    let edge_indices = polya_urn(x.len(), x.num_edges(undirected), undirected, loops, redraws);
                    GraphIID::new(x.len(), edge_indices, EmptyCodec::default(), EmptyCodec::default(), undirected)
                }).collect();
    ...
    fn node_labelled() {
        ...
        test_graph_shuffle_codecs(with_node_labelled_uniform(graphs.into_iter().take(20), 0.1, true, false), &TestConfig::test(0));
    }

    #[test]
    fn edge_labelled() {
        let graphs = dataset("MUTAG").edge_labelled_graphs().unwrap();
        test_graph_shuffle_codecs(with_edge_labelled_uniform(graphs.into_iter().take(20), 0.1, true, false), &TestConfig::test(0));
    }
