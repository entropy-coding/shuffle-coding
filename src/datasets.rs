...
use crate::graph::{DiGraph, EdgeIndex, Undirected, UnGraph};
use crate::permutable::Permutable;
use crate::plain::labelled_graph::{EdgeLabelledGraph, NodeLabelledGraph};
...
impl SourceInfo {
    ...
    pub fn defaults() -> Vec<SourceInfo> {
        lazy_static! {static ref D: Vec<SourceInfo> = vec![
            ...
            SourceInfo::new(
                "REC",
                "https://raw.githubusercontent.com/entropy-coding/rec-graphs/main/datasets.md"),
        ];}
        D.clone()
    }
}
...
mod tests {
    ...
    use crate::codec::{Independent, Message};
    use crate::datasets::dataset;
    use crate::graph_codec::{EmptyCodec, erdos_renyi_indices, GraphIID, polya_urn};
    use crate::graph_codec::tests::{test_graph_shuffle_codecs, test_joint_shuffle_codecs};
    use crate::permutable::{Permutable, Unordered};
    use crate::plain::labelled_graph::tests::{with_edge_labelled_uniform, with_node_labelled_uniform};
