...
use crate::permutable::{Automorphism, GroupPermutable, Orbit, Permutable, Permutation, PermutationGroup, StabilizerChain};
...
impl Codec for PermutationUniform {
    ...
    fn push(&self, m: &mut Message, x: &Self::Symbol) {
        ...
        let mut p_inv = Permutation::identity();
        let mut js = vec![];
        for i in (0..self.len).rev() {
            let xi = x * i;
            let j = &p_inv * xi;
            p_inv.swap_elements(&p * i, xi);
            p.swap_elements(i, j);
            js.push(j);
        }

        for (i, j) in js.iter().rev().enumerate() {
            Uniform::new(i + 1).push(m, j);
        }
    }
...
pub struct TestConfig {
    ...
    /// Test interleaved shuffle codec on uniform model.
    #[clap(skip)]
...
impl TestConfig {
    ...
    pub fn test(seed: usize) -> Self {
        Self { axioms_seed: Some(seed), blockwise: true, coset_interleaved: false, interleaved: false, seed: Some(seed) }
    }
