...
/// Maps nonconsecutive elements to consecutive masked elements.
/// OPTIMIZE: Use tree for faster removal.
#[derive(Clone, Debug)]
struct Mask {
...
impl Mask {
    fn unmasked(len: usize) -> Mask {
        let mask = (0..len).enumerate().collect();
        let unmask = (0..len).collect();
        Mask { mask, unmask }
    }

    fn len(&self) -> usize { self.unmask.len() }

    fn remove_element(&mut self, element: usize) {
        // Remove element and shift masked labels to keep them consecutive:
        let masked_elem = self.mask.remove(&element).unwrap();
        for m in self.mask.values_mut() {
            if *m > masked_elem {
                *m -= 1;
            }
        }
        assert_eq!(self.unmask.remove(masked_elem), element);

        assert_eq!(self.mask.len(), self.unmask.len());
        for (i, c) in self.unmask.iter().enumerate() {
            assert_eq!(self.mask[c], i);
        }
    }
}

#[derive(Clone)]
pub struct MaskedUniform {
    mask: Mask,
}

impl MaskedUniform {
    pub fn unmasked(len: usize) -> MaskedUniform {
        MaskedUniform { mask: Mask::unmasked(len) }
    }

    fn inner_codec(&self) -> Uniform {
        Uniform::new(self.mask.len())
    }

    fn remove_element(&mut self, element: usize) {
        self.mask.remove_element(element);
    }
}

impl Codec for MaskedUniform {
    type Symbol = usize;

    fn push(&self, m: &mut Message, x: &Self::Symbol) {
        let masked = self.mask.mask[x];
        self.inner_codec().push(m, &masked);
    }

    fn pop(&self, m: &mut Message) -> Self::Symbol {
        let masked = self.inner_codec().pop(m);
        self.mask.unmask[masked]
    }

    fn bits(&self, _: &Self::Symbol) -> Option<f64> { Some(self.uni_bits()) }
}

impl UniformCodec for MaskedUniform {
...
/// TODO consider recursive implementation
#[derive(Clone)]
pub struct InterleavedRCosetUniform {
    pub group: PermutationGroup,
    pub stab_codec: MaskedUniform,
}
...
impl InterleavedRCosetUniform {
    pub fn new(group: PermutationGroup) -> Self {
        let stab_codec = MaskedUniform::unmasked(group.len);
...
pub mod tests {
    use crate::ans::Codec;
    use crate::graph_ans::tests::small_graphs;
