...
use crate::ans::{Bernoulli, Codec, ConstantCodec, IID, Message, MutCategorical, MutDistribution};
...
use crate::recursive::complete::{multiset_shuffle_codec, MultisetShuffleCodec};
...
pub mod tests {
    ...
    use crate::graph::{DiGraph, Undirected, UnGraph};
    use crate::permutable::Permutable;
    use crate::plain::{PlainPermutable, with_isomorphism_test_max_len};
    use crate::plain::test::test_plain_shuffle_codecs;
    use crate::recursive::{Autoregressive, JointPrefix, JointSliceCodecs, ShuffleCodec};
    ...
    pub fn test_joint_shuffle_codecs<N: OrdSymbol, E: OrdSymbol, Ty: EdgeType, C: Codec<Symbol=Graph<N, E, Ty>> + Symbol + Len>(
        codecs: &Vec<C>,
        ...
        GraphPrefix<N, E, Ty>: PlainPermutable,
    {
        ...
        if config.joint {
            if config.complete {
                let complete_joint = |c| ShuffleCodec::new(JointSliceCodecs::new(c), CompleteOrbitCodecs::new());
    ...
    pub fn test_graph_shuffle_codecs<NodeC: Codec<Symbol: OrdSymbol> + Symbol, EdgeC: Codec<Symbol: OrdSymbol> + Symbol, Ty: EdgeType>(
        ...
        <GraphIID<NodeC, EdgeC, ErdosRenyi<Ty>> as Codec>::Symbol: PlainPermutable,
        ...
        with_isomorphism_test_max_len(config.isomorphism_test_max_len, || {
            ...
            if config.recursive {
                ...
                if config.ae {
                    ...
                    if config.complete {
                        let codecs = aer.clone().map(|c| ShuffleCodec::new(c.0, CompleteOrbitCodecs::new()));
                ...
                if config.ap && !Ty::is_directed() {
                    ...
                    if config.complete {
                        let codecs = apu.clone().map(|c| ShuffleCodec::new(c.0, CompleteOrbitCodecs::new()));
                        test_and_print_vec(codecs, unordered, &config.initial_message());
                    }
                    if !config.no_incomplete {
